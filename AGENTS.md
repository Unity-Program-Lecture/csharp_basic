...
8. 주요 변경 사항이나 수업 진행 상황은 `Documents/HISTORY.md`에 기록합니다.
9. 교육 과정의 상세 일정 및 일수 계산은 `Documents/COURSE_SCHEDULE_SUMMARY_202605141510.md`를 최우선으로 참조합니다.
10. 정교한 기하학적 표현이나 겹침 등 시각적 설명이 필요한 경우 별도 `.svg` 파일로 저장하고 Markdown 이미지 태그로 삽입합니다. GitHub 호환성을 위해 표준 SVG 네임스페이스를 포함하고, 다크/라이트 테마 모두에서 보이는 중간 톤 이상의 고대비 색상을 사용합니다.
