...
## 5. 5일차 심화 미션: "몬스터 무리와의 전투"

**[미션 목표]**
단일 몬스터와의 전투를 넘어, 배열을 통해 여러 마리의 몬스터 무리(Horde)를 관리하고 처리하는 시스템을 설계합니다. 반복문과 배열의 인덱스를 활용하여 다수의 객체를 효율적으로 다루는 능력을 기릅니다.
...
#### 1. 몬스터 군단 생성 (`Array`)
* `Monster[] monsters = new Monster[3];`와 같이 배열을 선언하고, 각 인덱스에 서로 다른 자식 객체(Slime, Orc, Dragon)를 할당합니다.

#### 2. 전체 상태 브리핑
* 반복문을 사용하여 현재 배열에 담긴 모든 몬스터의 이름과 남은 체력을 한 줄씩 출력합니다.

#### 3. 타겟팅 시스템
* 사용자로부터 인덱스 번호(0~2)를 입력받아 특정 몬스터만 공격합니다.
* 예: "몇 번 몬스터를 공격하시겠습니까? (0~2)" -> "1번 오크를 공격했습니다!"

#### 4. 생존 확인 및 승리 조건
* 공격 후 체력이 0 이하가 된 몬스터는 "처치됨" 상태로 표시하고, 더 이상 공격받지 않도록 예외 처리를 고민해 봅니다.
* 모든 몬스터의 체력이 0이 되면 "전투 승리!" 메시지를 출력하고 프로그램을 종료합니다.

---

### 2) 프로그래밍 힌트
* `for` 문이나 `foreach` 문을 사용하여 배열을 순회하며 상태를 출력해 보세요.
* `monsters[inputIndex].Hp > 0` 조건을 활용하여 살아있는 몬스터에게만 데미지를 입히도록 설계합니다.
* `Random` 클래스를 사용하여 플레이어의 데미지에 약간의 변동폭(예: 공격력 ±5)을 주면 더욱 게임다워집니다.


**[심화 과제 (선택 사항)]**
- **광역 공격 구현**: 모든 몬스터에게 동시에 동일한 데미지를 입히는 `SplashAttack()` 로직을 구현해 보세요.
- **랜덤 스폰**: 배열의 각 칸에 어떤 몬스터가 들어갈지 `Random`을 이용해 결정해 보세요. (예: 1~3 사이 숫자에 따라 슬라임/오크/드래곤 할당)
