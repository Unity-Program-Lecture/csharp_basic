...
3. **게임 UI/UX 프로그래밍 과정 학습 문서 6일 구성 추가**
   ...
   - HUD, 메뉴 화면 전환, UI 입력, Toast 알림 프레임워크, 통합 체크리스트, 포트폴리오 과제를 포함해 설계-구현-검증 흐름을 정리함.

2. **게임 시스템 기획 과정 학습 문서 6일 구성 추가**
   - `Documents/Curriculum/06.GameSystemPlanning/` 폴더를 추가하고 게임 시스템 기획 과정의 DAY 01~06 학습 문서를 작성함.
   - NCS `게임 시스템 기획` 모듈의 시스템 규칙, 시스템 공식, 시스템 테이블 흐름을 바탕으로 코어 메카닉스, 흐름도, 형식적 요소, 공식/밸런스, 데이터 테이블, 최종 시스템 기획서 리뷰로 재구성함.
   - 코딩 실습 대신 시스템 분석표, 텍스트 흐름도, 공식 설계서, 데이터 테이블, 리뷰 체크리스트를 작성하는 설계 문서 실습 중심 과정으로 정리함.

1. **게임 사전기획 과정 학습 문서 6일 구성 추가**
   - `Documents/Curriculum/05.GamePreProduction/` 폴더를 추가하고 게임 사전기획 과정의 DAY 01~06 학습 문서를 작성함.
   - NCS `게임 사전기획과 콘셉트기획` 모듈을 바탕으로 시장/트렌드 분석, 타겟 사용자와 수익 모델, 장르/콘텐츠 분석, 아이디어 제안서, 세계관/시나리오, 캐릭터/배경/최종 발표 흐름으로 재구성함.
   - 코딩 실습 대신 시장 분석표, 페르소나, 유사 게임 분석표, 아이디어 제안서, 설정서, 발표 자료를 작성하는 문서 실습 중심 과정으로 정리함.
