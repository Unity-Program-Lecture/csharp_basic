...
2. 3D 요소와 이펙트 공간 배치 분석
   - 이펙트 1개 이상에 발생 위치, 방향, 크기, 카메라 거리, 지속 시간을 정리합니다.
   - 해당 배치가 공격, 피격, 회복, 목표 달성 등 어떤 플레이 규칙을 읽히게 하는지 적습니다.
...
| 이펙트의 위치, 방향, 크기, 카메라 거리, 지속 시간을 분석해 기록했다. |  |
| 이펙트가 전달할 플레이 규칙과 발생 조건을 설명할 수 있다. |  |
