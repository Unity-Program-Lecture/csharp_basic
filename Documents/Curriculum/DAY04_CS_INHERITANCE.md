...
### 📋 생성자 상속 및 호출 규칙 요약
생성자는 일반 메소드와 달리 **자식에게 상속되지 않습니다.** 따라서 자식만의 생성자를 어떻게 정의하느냐에 따라 호출 방식이 달라집니다.

| 부모 생성자 구성 | 자식 생성자 정의 방식 | 결과 |
| :--- | :--- | :--- |
| **기본 + 오버로딩** | **아무것도 안 함** | 자식의 기본 생성자가 자동 생성되며, 부모의 **기본 생성자**를 호출함. |
| **기본 + 오버로딩** | **오버로딩만 정의** | 자식은 오버로딩된 방식으로만 생성 가능. 부모의 **기본 생성자**가 자동 호출됨. |
| **기본 + 오버로딩** | **오버로딩 + `base()`** | 자식은 오버로딩된 방식으로 생성. 부모의 **오버로딩된 생성자**를 선택해서 호출함. |
| **오버로딩만 있음** | **반드시 정의 필요** | 부모에게 기본 생성자가 없으므로, 자식은 반드시 생성자를 만들고 `base(...)`를 써야 함. |

> **핵심 팁**: 자식 생성자에서 `: base(...)`를 적지 않으면, 컴파일러는 무조건 부모의 **매개변수 없는 기본 생성자**를 찾으려고 시도합니다. 부모에게 기본 생성자가 없다면 여기서 에러가 발생합니다!
