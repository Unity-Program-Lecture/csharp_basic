...
## 2. GUI 디자인 가이드와 구현 가능성 확인

GUI 디자인 가이드는 "버튼은 이 크기, 경고는 이 색, 같은 종류의 글자는 이 글꼴"처럼 화면의 공통 규칙을 적어 둔 약속입니다. 이를 먼저 보면 디자인을 Unity 6에서 실제로 만들 수 있는지 판단할 수 있습니다.

| 가이드 항목 | 초안 규칙 | Unity 6에서 확인할 방법 | 변경이 필요한 경우 |
| :--- | :--- | :--- | :--- |
| 최소 버튼 크기 | 마우스로 누르기 쉽게 충분한 크기로 둔다. | Game View에서 클릭하고 게임패드 선택 테두리를 확인 | 작은 화면에서 글자가 잘리면 버튼 높이 또는 글자 크기를 조정 |
| 글자 대비 | 배경과 글자가 분명히 구분된다. | 밝은 패널과 어두운 패널 모두에서 읽기 확인 | 읽기 어려우면 글자색, 배경색, 외곽선 중 하나 변경 |
| 정보 우선순위 | 체력·시간처럼 급한 정보가 먼저 보인다. | 플레이 화면에서 시선 이동을 관찰 | 중요하지 않은 장식이 먼저 보이면 위치 또는 크기 변경 |
| 지원 입력 | 마우스 외 키보드/게임패드도 조작 가능 | EventSystem과 Navigation을 연결해 테스트 | 선택 순서가 어색하면 Navigation을 Explicit으로 변경 |

**설계 변경 판단 예시:** 와이어프레임에서 버튼을 가로로 다섯 개 배치했는데 `1024 x 768`에서 글자가 겹친다면, "억지로 원안을 유지"하지 않습니다. 버튼을 두 줄로 나누거나 설정 화면을 별도 Panel로 분리하는 것이 GUI 프로세스를 구현 환경에 맞게 조정한 결과입니다.
...
UI를 만든 뒤에는 "보였다"에서 끝내지 말고, 가이드와 실제 결과를 비교해 기록합니다.

| 요소 | 가이드 또는 기대 결과 | 테스트 결과 | 설계 변경 여부 |
| :--- | :--- | :--- | :--- |
| 점수 Text | 왼쪽 위에 고정되고 숫자가 잘리지 않는다. |  |  |
| 체력 Slider | 왼쪽 아래에 고정되고 0 아래로 내려가지 않는다. |  |  |
| 시간 Text | 위쪽 가운데에 고정되고 0 아래로 내려가지 않는다. |  |  |
| 테스트 Button | 마우스로 클릭할 수 있고 상태 변화가 보인다. |  |  |

빈칸에는 `통과`, `수정 후 통과`, `수정 필요`처럼 실제 확인 결과를 적습니다. 수정이 필요하면 Anchor, 글자 영역, Canvas Scaler 설정 중 무엇을 바꿨는지도 한 줄로 남깁니다.
...
- GUI 디자인 가이드는 꾸미기 목록이 아니라 구현 가능성과 설계 변경을 판단하는 기준입니다.
