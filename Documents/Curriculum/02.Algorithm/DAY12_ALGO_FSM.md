# 🚀 Day 12: 게임 알고리즘 기초 (유한 상태 머신, FSM)
...
## 🎯 [심화 미션] 몬스터 사냥 시스템: 보스 몬스터의 다단계 패턴
### [요구 사항]
- 보스 몬스터의 체력(HP) 비율에 따라 행동 패턴이 변하는 '2단계(Phase 2) 시스템'을 FSM으로 기획하세요.
- 1단계에서는 '원거리 공격' 위주로 행동하다가, 체력이 50% 이하가 되면 2단계 상태로 전이하여 '광폭화 및 근접 돌진' 패턴을 수행해야 합니다.
- 각 상태 내에서 '플레이어와의 거리'라는 추가 조건을 확인하여 공격과 추적 상태를 전환하는 로직을 구상하세요.

### [프로그래밍 힌트]
- `switch-case` 문 내부에서 HP 조건을 체크하여 상태 변수(`currentState`)를 변경할 수 있습니다.
- 각 상태를 별도의 클래스로 분리하는 '상태 패턴(State Pattern)'의 구조도 함께 고민해 보세요.
