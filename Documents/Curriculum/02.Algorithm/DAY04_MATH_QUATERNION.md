...
이 미션에서 "비행 몬스터"는 실제로 날아다니는 이동 AI를 만들라는 뜻이 아닙니다. 씬 위쪽에 고정해 둔 몬스터 오브젝트가 아래쪽에 있는 플레이어 오브젝트를 바라보도록 회전만 구현합니다.

### [준비]
- `FlyingMonster`라는 큐브 또는 캡슐을 만들고 위치를 `(0, 5, 0)` 근처에 둡니다.
- `PlayerTarget`이라는 큐브를 만들고 위치를 `(3, 0, 4)` 근처에 둡니다.
- 이번 미션에서는 몬스터 이동, 중력, 공격 발사체, 애니메이션은 구현하지 않습니다.

### [요구 사항]
- 공중에 고정된 `FlyingMonster`가 지상에 있는 `PlayerTarget`을 부드럽게 바라보도록 회전시키세요.
- `PlayerTarget`을 몬스터의 거의 아래쪽으로 옮겨도 회전이 갑자기 튀지 않아야 합니다.
- 단순히 방향만 보는 것이 아니라, 몬스터의 위쪽 기준 (Roll) 이 지면의 위쪽 방향과 크게 어긋나지 않도록 조준해야 합니다.
- Scene 뷰에서 현재 앞 방향과 목표 방향을 기즈모 선으로 비교할 수 있게 만드세요.

### [프로그래밍 힌트]
- `Vector3 direction = player.position - transform.position;`으로 몬스터에서 플레이어를 향하는 방향을 구합니다.
