# 🚀 Day 13: 게임 알고리즘 - 탐색과 길찾기 (BFS & DFS)
...
## 🎯 [심화 미션] 몬스터 사냥 시스템: 무리 사냥과 포위 전술
### [요구 사항]
- 무리지어 다니는 '늑대 몬스터'들이 플레이어를 발견했을 때, 가장 가까운 늑대가 먼저 다가가고 나머지 늑대들이 주변 노드들을 점유하여 포위망을 형성하는 시나리오를 기획하세요.
- 그래프 탐색을 통해 플레이어 주변의 빈 거점(Node)들을 빠르게 찾아내는 방식에 어떤 알고리즘(BFS/DFS)이 더 효율적일지 판단하세요.
- 이미 다른 늑대가 점유한 노드는 탐색에서 제외하는 '방문 처리' 로직을 설계하세요.

### [프로그래밍 힌트]
- 최단 거리에 있는 주변 노드들을 넓게 퍼지며 찾는 데는 `BFS`가 유리합니다.
- `Queue`에 플레이어의 현재 위치 노드를 넣고 탐색을 시작해 보세요.
