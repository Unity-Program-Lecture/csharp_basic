...
## 🎯 [심화 미션] 몬스터 사냥 시스템: 정찰 구역 노드 매핑
### [요구 사항]
- 몬스터가 마을의 주요 거점들을 순찰하기 위한 '정찰 경로 그래프'를 설계하세요.
- 각 거점(Node) 사이의 이동 거리(Weight)를 행렬 또는 리스트 형태로 관리하여, 가장 가까운 다음 거점을 찾는 로직을 구성하세요.
- 특정 경로가 무너졌을 때(Edge 제거), 다른 우회 경로를 통해 순찰을 계속할 수 있는 유연한 구조를 기획하세요.

### [프로그래밍 힌트]
- 2차원 배열을 이용한 '**인접 행렬** (Adjacency Matrix)' 방식이나 `List`를 이용한 '**인접 리스트**' 방식을 활용할 수 있습니다.
- `Dictionary<string, List<string>>`을 사용해 거점 이름과 연결된 거점들을 매핑해 보세요.
