...
본 강의 내용은 **"게임 알고리즘 프로그래밍(NCS 0803020526_18v4)"**의 **수학/물리 공간 알고리즘 응용** 및 **평가 준거**를 완벽하게 충족합니다.

| NCS 평가 준거 | 학습 대응 영역 | 만족 기법 및 로직 |
| :--- | :--- | :--- |
| **물리 환경 연산 설계** | 중력 및 가속도에 의한 운동량 보존과 궤적 감쇠 | 오일러 적분 수식 구현 및 Rigidbody Drag 물리계 수치 동기화 |
| **알고리즘 구현 검증** | 수학적 공식 기반 실시간 궤적 예측 및 렌더링 | `PredictTrajectory` 실시간 Raycast 충돌 판정 및 가상 프레임 루프 처리 |

---

## ✍️ 평가 문항 대비 핵심 퀴즈

1. **문제:** 투사체가 날아갈 때 공기 분자와의 마찰로 인해 속도가 느려지는 물리 현상을 무엇이라고 하며, 유니티 Rigidbody 컴포넌트에서 제어하는 속성은 무엇입니까?
   - **정답:** 공기 저항(항력 / Drag), `Rigidbody.drag`

2. **문제:** 공기 저항이 존재할 때 투사체의 포물선은 왜 대칭 구조를 이루지 못하고 후반부에 급격히 수직으로 낙하하게 되나요?
   - **정답:** 수평 속도($v_x$)가 항력($F_d = -cv_x$)에 의해 지속적으로 지수 감쇠하는 반면, 수직 속도는 중력 가속도($g$)에 의해 계속 하방으로 증가하여 궤적 후반부에는 수평 이동 거리보다 낙하 거리가 압도적으로 커지기 때문입니다.

3. **문제:** 궤적을 실시간으로 미리 그리기 위해(Predictive Trajectory) C#에서 물리 프레임을 가상으로 한 단계씩 전진시키는 수치 해석 계산 기법을 무엇이라 합니까?
   - **정답:** 오일러 적분(Euler Integration) 기법
