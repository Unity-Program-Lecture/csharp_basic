...
본 1일차 수업 내용은 NCS 게임 알고리즘 능력단위의 **"게임 수학 적용하기"** 수행준거와 직접적으로 연계됩니다.

### 1. 서술형 평가 대비 포인트
* **벡터의 뺄셈 기하학적 의미**: 몬스터가 플레이어를 추적하는 기능을 구현할 때, 대상 벡터에서 시작 벡터를 빼는 "**Target - Self**" 연산이 두 물체 사이의 방향과 거리를 동시 제공함을 논리적으로 기술해야 합니다.
* **벡터의 정규화(Normalization)**: 크기가 1인 단위 벡터(`Vector3.normalized`)가 이동 방향만 유지한 채 매 프레임 등속도로 이동을 처리하기 위해 왜 필요한지 스칼라 속도값과의 곱셈 원리를 들어 설명해야 합니다.

### 2. 포트폴리오(실기) 평가 포인트
* 키보드 입력을 통해 플레이어가 3D 공간 상에서 정규화된 방향 벡터(`moveDir.normalized`) 및 `Time.deltaTime`을 기반으로 등속 이동하는 C# 스크립트 작성 능력을 평가합니다. (Unity 6 신형 Input System 네임스페이스 및 `Keyboard.current` 활용 검증)
