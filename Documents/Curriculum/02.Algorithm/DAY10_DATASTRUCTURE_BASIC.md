...
## 🎯 [심화 미션] 몬스터 사냥 시스템: 스킬 연계 및 행동 대기열
### [요구 사항]
- 보스 몬스터가 사용하는 여러 스킬들을 순차적으로 실행하기 위한 '행동 대기열(Action Queue)' 시스템을 자료구조를 활용해 설계하세요.
- 몬스터가 플레이어에게 입은 최근 피해 기록 5개를 보관하고, 가장 오래된 기록부터 삭제되는 시스템에 어떤 자료구조가 적합할지 결정하세요.
- 특정 조건(분노 상태)에서 가장 최근에 예약된 스킬을 취소하고 즉시 반격 스킬을 넣는 상황을 가정해 보세요.

### [프로그래밍 힌트]
- 순차적 스킬 실행에는 `Queue<T>`, 최근 피해 기록 관리에는 `List<T>` 또는 `Fixed-size Array`를 활용할 수 있습니다.
- 스킬 취소와 재배치가 빈번하다면 `Stack<T>`이나 `LinkedList<T>`의 특성도 고려해 보세요.
