...
이 문서는 게임 데이터베이스 실습에서 필요한 `Microsoft.Data.Sqlite`, `LiteDB`처럼 **내 프로젝트에 기본으로 들어 있지 않은 기능**을 추가하는 방법을 설명합니다. 4일차와 6일차 실습 전에 필요할 때 찾아봅니다.
...
> 이 문서의 패키지 설치 방법은 4~7일차의 **.NET 콘솔 프로젝트** 기준입니다. Unity 프로젝트에서 NuGet 패키지를 바로 추가하는 일은 호환성·플랫폼 빌드 확인이 필요하므로, 이 과정에서는 Unity가 DB 결과를 표시하는 역할에 집중합니다.
...
> **Visual Studio의 프로젝트 생성**과 **Unity 프로젝트 생성**은 다른 작업입니다. 4~7일차의 DB 코드는 우선 Visual Studio 콘솔 프로젝트에서 실행합니다.
