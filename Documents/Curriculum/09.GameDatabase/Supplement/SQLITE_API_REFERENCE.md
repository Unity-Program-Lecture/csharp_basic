...
| 코드 조각 | 뜻 | 사용할 때 |
| :--- | :--- | :--- |
| `CREATE TABLE` | 새 표를 만든다 | DB 구조를 처음 준비할 때 |
| `INSERT INTO ... VALUES` | 지정한 열에 새 행의 값을 넣는다 | 아이템·플레이어 데이터를 추가할 때 |
| `SELECT ... FROM` | 표에서 필요한 열을 조회한다 | 목록이나 값을 읽을 때 |
| `WHERE 조건` | 조건에 맞는 행으로 대상을 좁힌다 | 특정 플레이어·가격 범위만 찾을 때 |
| `UPDATE ... SET` | 조건에 맞는 행의 값을 바꾼다 | 골드나 수량을 변경할 때 |
| `DELETE FROM` | 조건에 맞는 행을 삭제한다 | 테스트 데이터를 정리할 때 |
