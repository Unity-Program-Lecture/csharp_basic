...
DAY01~02에서 배운 문장을 C# 문자열 안에서도 같은 순서로 읽습니다.

| 코드 조각 | 뜻 | 이 코드에서 하는 일 |
| :--- | :--- | :--- |
| `CREATE TABLE IF NOT EXISTS` | 표가 없을 때만 만든다 | 프로그램을 두 번 실행해도 표 생성 오류를 피함 |
| `PRAGMA foreign_keys = ON` | SQLite 연결의 외래 키 검사를 켠다 | 없는 플레이어·아이템 번호 연결을 검사함 |
| `INSERT OR IGNORE` | 넣되 같은 기본 키가 있으면 넘어간다 | 초기 플레이어와 포션을 한 번만 준비함 |
