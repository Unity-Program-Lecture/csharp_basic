# DAY 04: 문서형 NoSQL DB 설계와 관계형 DB 비교 (4교시)
...
## 2. NoSQL에는 어떤 유형이 있나요?

| NoSQL 유형 | 데이터를 보는 방법 | 대표 제품 | 게임에서 떠올릴 수 있는 예 |
| :--- | :--- | :--- | :--- |
| 문서형 | JSON과 비슷한 문서 한 건 | LiteDB, MongoDB | 퀘스트 진행, 유연한 오류 로그 |
| 키-값형 | 키 하나와 값 하나 | Redis | 짧은 시간 캐시, 세션 정보 |
| 컬럼형 | 많은 행을 열 중심으로 분산 저장 | Apache Cassandra, HBase | 대량 이벤트 기록 분석 |
| 그래프형 | 노드와 연결 관계 | Neo4j | 친구 관계, 추천 관계 |

이 표는 분류를 이해하기 위한 것입니다. 모든 NoSQL을 실습하지는 않습니다. 이 과정의 실제 NoSQL 실습은 문서형 LiteDB로 한정합니다.
