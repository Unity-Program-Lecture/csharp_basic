...
오늘은 .NET (닷넷) 프로그램용 LiteDB 패키지를 C# 프로젝트에 추가하고, 게임 로그 문서를 생성·조회합니다. .NET은 C# 프로그램을 빌드하고 실행하는 개발 플랫폼입니다.

## NCS 연결

- 능력단위 요소: 게임 데이터베이스 프로그래밍하기
- 주요 학습 내용: 문서형 NoSQL DB를 생성하고 관리하는 프로그램 작성하기

## 1. LiteDB는 무엇인가요?

LiteDB는 .NET 프로그램 안에서 사용하는 파일 기반 **문서형 NoSQL DB**입니다. NoSQL에는 문서형 외에도 키-값형, 컬럼형, 그래프형이 있으며, LiteDB는 그중 JSON과 비슷한 문서를 저장하는 문서형에 속합니다. 별도의 DB 서버 프로그램을 설치하지 않고 NuGet 패키지를 프로젝트에 추가합니다.
