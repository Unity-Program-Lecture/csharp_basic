# DAY 18: 게임엔진 응용 프로그래밍 정리

오늘의 목표는 18일 동안 배운 내용을 "**작은 게임 제작 체크리스트**"로 다시 묶고, Unity 프로젝트를 스스로 점검하는 방법을 정리하는 것입니다.
...
- **Play Test**: 실제로 실행하며 카메라, 이동, 물리, UI, 사운드가 이어지는지 확인하는 과정입니다.
...
| 카메라 | Main Camera가 주요 공간과 오브젝트를 잘 보여 주나요? |
| 입력 | 플레이어 조작이 의도대로 작동하나요? |
| 물리 | Collider와 Rigidbody 설정이 맞나요? |
| 이동 | Character Controller 또는 Navigation 이동이 끊기지 않나요? |
| 애니메이션 | 이동이나 상태 변화에 맞춰 Animator State가 전환되나요? |
| UI | 점수, 버튼, 안내 문구가 화면에 잘 보이나요? |
| 사운드/VFX | 중요한 행동에 피드백이 있나요? |
| 에셋 | 외부 에셋과 직접 만든 에셋이 구분되어 있나요? |
| Inspector | 핵심 컴포넌트의 주요 프로퍼티가 의도대로 설정되어 있나요? |
...
- 게임엔진 수업의 핵심은 GameObject, Component, Scene, 카메라, 이동, 물리, UI의 연결입니다.
