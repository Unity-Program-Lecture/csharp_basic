...
### 💻 실습 예제: 간단한 계산기 만들기
**미션:** 두 개의 정수를 사용하여 사칙연산과 나머지 연산, 그리고 비교 및 논리 연산의 결과를 출력하는 프로그램을 만들어 보세요.
...
### 💻 실습 예제: 럭키 세븐 찾기
**미션:** 1에서 10 사이의 숫자들 중에서 짝수는 건너뛰고(continue), 홀수만 출력하다가 7을 찾으면 반복을 멈추는(break) 코드를 작성해 보세요.
...
### 💻 실습 예제: 무기 상점
**미션:** 사용자로부터 1 또는 2를 입력받아 선택한 무기에 따른 메시지를 출력하고, 그 외의 값을 입력하면 안내 메시지를 보여주는 메뉴 선택 시스템을 만들어 보세요.
...
**미션:** 몬스터의 초기 HP가 30일 때, HP가 0보다 큰 동안 반복해서 공격하며 HP를 10씩 감소시키는 과정을 출력해 보세요.
...
### 💻 실습 예제: 내년 나이 구하기
**미션:** 사용자로부터 문자열로 나이를 입력받아 숫자로 변환(Parse)한 뒤, 내년 나이를 계산하여 출력하는 프로그램을 작성해 보세요.
...
**미션:** 정해진 정답 숫자를 맞출 때까지 사용자에게 숫자를 입력받고, 정답보다 큰지 작은지 힌트를 주며 맞추면 종료되는 게임을 구현해 보세요.
...
---
## 6. 실전 미션: "간이 자판기 프로그램" 구현하기
**미션:** 사용자로부터 돈을 입력받고, 메뉴를 선택하여 물건을 구매하는 프로그램을 작성합니다. 잔액이 부족하면 경고 메시지를 띄우고, 사용자가 원할 때까지 반복해서 구매할 수 있어야 합니다.

1. 요구 사항
    - 초기 설정: 프로그램이 시작되면 사용자의 현재 잔액(balance)은 0원입니다.
    - 금액 충전: 먼저 사용자에게 투입할 금액을 입력받습니다.
    - 메뉴 표시: 다음과 같은 메뉴와 가격을 화면에 출력합니다.
        - 콜라 (1,200원)
        - 사이다 (1,100원)
        - 커피 (700원)
        - 잔돈 반환 및 종료
    ...
    - 구매 로직:
        - 사용자가 번호를 선택하면 해당 물품의 가격만큼 잔액에서 차감합니다.
        - 조건: 잔액이 물품 가격보다 적다면 "잔액이 부족합니다!"라는 메시지를 출력하고 차감하지 않습니다.
        - 반복: 4번을 누르기 전까지는 계속해서 물건을 구매하거나 잔액을 확인할 수 있습니다.
        - 종료: 4번을 누르면 남은 잔액을 출력하며 프로그램을 종료합니다.
...
## ✍️ 2일차 핵심 퀴즈
1. `10 / 3`의 결과값은 얼마인가요? (데이터 타입을 생각해보세요!)
